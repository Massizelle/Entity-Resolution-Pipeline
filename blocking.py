...
def run_token_blocking(
    ...
    n_in_blk  = df_blocks["entity_id"].nunique()
...
def compute_blocking_stats(
    ...
    for _, grp in df_blocks.groupby("block_id"):
        s1_ids = set(grp[grp["source"] == source1_name]["entity_id"])
        s2_ids = set(grp[grp["source"] == source2_name]["entity_id"])
        n_candidates += len(s1_ids) * len(s2_ids)

        if true_pairs:
            for a in s1_ids & true_s1_ids:
                for b in s2_ids & true_s2_ids:
