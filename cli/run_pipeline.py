...
Usage:
    ...
    # Tuning parameters
    python cli/run_pipeline.py --max-block-size 500 --min-jaccard 0.1
...
def _render_progress_bar(current: int, total: int, *, width: int = 28, color: str = "36") -> str:
    total = max(1, total)
...
def _print_dataset_progress(dataset: str, completed_steps: int, total_steps: int) -> None:
    print(
        f"\n  Pipeline progress — {dataset}: "
        f"{_render_progress_bar(completed_steps, total_steps, color='32')}"
    )


def _print_global_pipeline_progress(completed_units: int, total_units: int) -> None:
    print(
        f"\n  Global progress: "
        f"{_render_progress_bar(completed_units, total_units, color='35')}"
    )
...
def step2_block_processing(
    ...
    *,
    ...
    """Member 2: block purging + witness-first candidate generation → candidate_pairs.csv."""
    cfg = DATASET_REGISTRY[dataset]
...
def step4_clustering(
    ...
    *,
    ...
    print(f"  [CLUST] {n_matches:,} confirmed matches → clustering with 'connected_components'")
...
def run_pipeline(
    ...
    *,
    ...
    total_pipeline_units = len(datasets) * sum(1 for step in range(from_step, to_step + 1) if 1 <= step <= 4)
    completed_pipeline_units = 0
    if total_pipeline_units > 0:
        _print_global_pipeline_progress(completed_pipeline_units, total_pipeline_units)

    for ds_key in datasets:
        banner(f"DATASET: {ds_key.upper()}")
        ds_stats: dict[str, Any] = {}
        planned_steps = sum(1 for step in range(from_step, to_step + 1) if 1 <= step <= 4)
        completed_steps = 0
        _print_dataset_progress(ds_key, completed_steps, planned_steps)
        ...
        if from_step <= 2 <= to_step:
            step_banner(2, "Block Purging + Witness-First Candidate Generation  [Member 2]")
            s = step2_block_processing(
                ds_key, use_mock, max_block_size,
                candidate_strategy="cw_semantic_predictive",
