...
Usage:
    ...
    python cli/run_member1.py --mocks-only

    # Full pipeline — all three datasets
    python cli/run_member1.py

    # One dataset only
    python cli/run_member1.py --dataset abt_buy
    python cli/run_member1.py --dataset amazon_google
    python cli/run_member1.py --dataset dblp_acm
    python cli/run_member1.py --dataset rexa_dblp
    python cli/run_member1.py --dataset spimbench
...
def run_pipeline(
    ...
    for ds_key in datasets_to_run:
        ...
        try:
            ...
            if skip_missing:
                print(f"  [SKIP] {exc}")
                print(f"  [SKIP] Dataset '{ds_key}' skipped.")
                continue
            raise
        ...
        all_stats[ds_key] = {
            ...
            "n_entities":    df_blocks.groupby("source")["entity_id"].nunique().sum(),
            "stats":         stats,
        }
...
def main():
    ...
    parser.add_argument(
        ...
        help="Skip datasets whose raw data files cannot be found instead of raising an error"
    )
