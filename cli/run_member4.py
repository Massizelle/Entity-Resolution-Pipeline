...
then merges their attributes into canonical records.

Two clustering algorithms are supported:
  - connected_components  (default) : every connected component in the
      match graph becomes one cluster (transitive closure of matches).
  - center                          : greedy star-partition — the node
      with the highest degree is chosen as center, its neighbours form
      one cluster, then repeat on the remaining graph.

Usage:
    # All real datasets — connected components (default)
    ...
    python cli/run_member4.py --algorithm center
    python cli/run_member4.py --dataset abt_buy --algorithm connected_components
...
def run_clustering(
    ...
    *,
    ...
    print(f"  [CLUST] Running '{algorithm}' ...")
    t0 = time.time()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Clustering algorithm (default: connected_components)",
    )
    parser.add_argument(
        "--mock",
        action="store_true",
    ...
    for ds_key in datasets:
        banner(f"DATASET: {ds_key.upper()}  [algorithm={args.algorithm}]")
