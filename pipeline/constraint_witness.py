...
def _entity_title_texts(df: pd.DataFrame) -> dict[str, str]:
    ...
    for _, row in df.iterrows():
        ...
        if title_text.strip():
            texts[str(row["id"])] = title_text
            continue
        # Some benchmarks contain many rows with a missing/empty title on one
        # side (e.g. IMDb-like records that keep only cast/writer fields).  In
        # those cases the title-only view starves the downstream semantic and
        # rescue stages even though the row still contains informative text.
        texts[str(row["id"])] = _row_text(row, text_columns)
    return texts
...
def _build_lr_vectors(
    ...
    *,
    char_too: bool = False,
) -> "tuple[dict[str, dict[str, float]], dict[str, dict[str, float]]] | tuple[dict[str, dict[str, float]], dict[str, dict[str, float]], dict[str, dict[str, float]], dict[str, dict[str, float]]]":
    """Build TF-IDF (and optionally char-TF-IDF) vectors for left/right entities.

    Uses prefixed keys (``"L:<id>"`` / ``"R:<id>"``) when building the shared
    corpus so that overlapping integer IDs between sources (e.g. dblp_acm where
    DBLP and ACM both start from 0) do not overwrite each other.  The returned
    dicts are keyed by bare entity_id (without prefix) for convenient lookup.
    """
    combined: dict[str, str] = {}
    for eid in left_ids:
        text = texts_left.get(eid, "")
        if text.strip():
            combined[f"L:{eid}"] = text
    for eid in right_ids:
        text = texts_right.get(eid, "")
        if text.strip():
            combined[f"R:{eid}"] = text

    token_vecs_prefixed, _ = _sparse_tfidf_vectors(combined)
    left_token = {eid: token_vecs_prefixed.get(f"L:{eid}", {}) for eid in left_ids}
    right_token = {eid: token_vecs_prefixed.get(f"R:{eid}", {}) for eid in right_ids}

    if char_too:
        char_vecs_prefixed, _ = _sparse_char_tfidf_vectors(combined)
        left_char = {eid: char_vecs_prefixed.get(f"L:{eid}", {}) for eid in left_ids}
        right_char = {eid: char_vecs_prefixed.get(f"R:{eid}", {}) for eid in right_ids}
        return left_token, right_token, left_char, right_char  # type: ignore[return-value]

    return left_token, right_token  # type: ignore[return-value]
...
def _pair_feature_vector(
    ...
    *,
    ...
    left_vector = left_vectors.get(left_id, {})
    right_vector = right_vectors.get(right_id, {})
...
def extract_witnesses(
    ...
    *,
    ...
    scan_progress = ProgressBar(
        f"{progress_label} Extraction témoins / scan" if progress_label else "",
        len(combined),
        color="34",
        unit_label="entities",
    ...
    for idx, (_, row) in enumerate(combined.iterrows(), start=1):
        ...
        # Composite key prevents collision when both sources share the same integer
        # ID space (e.g. dblp_acm where DBLP and ACM both use IDs 0-N).
        entity_tokens[f"{entity_id}@{source_key}"] = tokens
        token_frequency.update(set(tokens))
        if scan_progress:
            scan_progress.update(idx)
    if scan_progress:
        scan_progress.close()

    rows: list[dict[str, Any]] = []
    build_progress = ProgressBar(
        f"{progress_label} Extraction témoins / build" if progress_label else "",
        len(combined) + (len(blocks_df) if blocks_df is not None else 0),
        color="34",
        unit_label="items",
    ) if progress_label else None
    for idx, (_, row) in enumerate(combined.iterrows(), start=1):
        ...
        tokens = entity_tokens[f"{entity_id}@{source}"]
        ...
        if build_progress:
            build_progress.update(idx)

    if blocks_df is not None and not blocks_df.empty:
        offset = len(combined)
        for idx, (_, row) in enumerate(blocks_df.iterrows(), start=1):
            entity_id = str(row["entity_id"])
            source = str(row["source"])
            ...
            if build_progress:
                build_progress.update(offset + idx)
    if build_progress:
...
def collapse_witness_regions(
    ...
    *,
    ...
    progress = ProgressBar(
        f"{progress_label} Collapse régions" if progress_label else "",
        max_region_visits,
        color="36",
        unit_label="visits",
    ) if progress_label else None
...
def semantic_rescue_pairs(
    ...
    *,
    ...
    left_vectors, right_vectors = _build_lr_vectors(
        texts_left, weak_left_ids, texts_right, right_ids
    )
    ...
    for left_id in weak_left_ids:
        ...
        ranked = sorted(
            (
                (_sparse_cosine(left_vector, right_vectors.get(right_id, {})), right_id)
    ...
    *,
    ...
    for left_id in weak_left_ids:
        ...
        for right_id, bucket in candidate_support.items():
            ...
            scored_pairs += 1
            semantic_score = _sparse_cosine(
                left_vectors.get(left_id, {}), right_vectors.get(right_id, {})
            )
...
def asymmetric_text_rescue_pairs(
    ...
    *,
    ...
    left_token_vecs, right_token_vecs, left_char_vecs, right_char_vecs = _build_lr_vectors(
        texts_left, weak_left_ids, texts_right, right_ids, char_too=True
    )
    entity_witnesses = _witness_sets_by_entity(witnesses_df)
    ...
    scored_pairs = 0
    for left_id in weak_left_ids:
        left_token_vector = left_token_vecs.get(left_id, {})
        left_char_vector = left_char_vecs.get(left_id, {})
        ...
        token_ranked = sorted(
            (
                (_sparse_cosine(left_token_vector, right_token_vecs.get(right_id, {})), right_id)
                for right_id in right_ids
        ...
        char_ranked = sorted(
            (
                (_sparse_cosine(left_char_vector, right_char_vecs.get(right_id, {})), right_id)
                for right_id in right_ids
                if (left_id, right_id) not in existing_pairs
            ),
            key=lambda item: (-item[0], item[1]),
        ...
        for right_id in shortlist:
            right_witnesses = entity_witnesses.get(right_id, set())
            token_score = _sparse_cosine(left_token_vector, right_token_vecs.get(right_id, {}))
            char_score = _sparse_cosine(left_char_vector, right_char_vecs.get(right_id, {}))
...
def facet_rescue_pairs(
    ...
    *,
    ...
    left_vectors_facet, right_vectors_facet = _build_lr_vectors(
        texts_left, weak_left_ids, texts_right, right_ids
    )
    entity_witnesses = _witness_sets_by_entity(witnesses_df)
    ...
    for left_id in weak_left_ids:
        ...
        left_vector = left_vectors_facet.get(left_id, {})
        ...
        if not candidate_ids:
            ranked = sorted(
                (
                    (_sparse_cosine(left_vector, right_vectors_facet.get(right_id, {})), right_id)
                    for right_id in right_ids
                    if (left_id, right_id) not in existing_pairs
                ),
                key=lambda item: (-item[0], item[1]),
        ...
        for right_id in candidate_ids:
            rf = right_facets[right_id]
            right_witnesses = entity_witnesses.get(right_id, set())
            semantic = _sparse_cosine(left_vector, right_vectors_facet.get(right_id, {}))
...
def predictive_rescue_pairs(
    ...
    *,
    ...
    entity_witnesses = _witness_sets_by_entity(witnesses_df)
    all_left_ids = [eid for eid, text in texts_left.items() if text.strip()]
    left_vectors, right_vectors = _build_lr_vectors(
        texts_left, all_left_ids, texts_right, right_ids
    )
...
def run_constraint_witness_resolution(
    ...
    *,
    ...
    phase_bar = ProgressBar(
        f"{progress_label} Sous-phases witness-first" if progress_label else "",
        6,
        color="35",
        unit_label="phases",
    ) if progress_label else None
    ...
    if phase_bar:
        phase_bar.update(1, detail="témoins extraits")
    ...
    if phase_bar:
        phase_bar.update(2, detail="collapse terminé")
    if semantic_rescue:
    ...
    if phase_bar:
        phase_bar.update(3, detail="semantic rescue terminé")
    if predictive_rescue:
        ...
        if phase_bar:
            phase_bar.update(4, detail="predictive rescue terminé")
        ...
        if phase_bar:
            phase_bar.update(5, detail="strong/asymmetric rescues")
    ...
    if phase_bar:
        phase_bar.update(6, detail="facet rescue terminé")
        phase_bar.close()
