...
TITLE_HINTS = ("title", "name", "label")
PERSON_HINTS = ("actor", "director", "writer", "editor", "starring", "author", "maker", "cast")
YEAR_HINTS = ("year", "date")
...
def normalize_block_text(text: str) -> str:
    text = _decode_unicode_escapes(str(text).lower())
...
def _title_columns(row: pd.Series, exclude_cols: set[str]) -> list[str]:
    return [
        col for col in row.index
        if col not in exclude_cols and any(hint in str(col).lower() for hint in TITLE_HINTS)
    ]


def _person_columns(row: pd.Series, exclude_cols: set[str]) -> list[str]:
    return [
        col for col in row.index
        if col not in exclude_cols and any(hint in str(col).lower() for hint in PERSON_HINTS)
    ]


def _year_value(row: pd.Series, exclude_cols: set[str]) -> str:
    for col in row.index:
        if col in exclude_cols:
            continue
        lowered = str(col).lower()
        if any(hint in lowered for hint in YEAR_HINTS):
            tokens = list(tokenize(str(row.get(col, "")), min_len=4))
            for token in tokens:
...
def _title_block_keys(row: pd.Series, exclude_cols: set[str]) -> set[str]:
    keys: set[str] = set()
    title_columns = _title_columns(row, exclude_cols)
    title_text = " ".join(str(row.get(col, "")) for col in title_columns if str(row.get(col, "")).strip())
    if not title_text:
        return keys

    title_text = re.sub(r"\b(?:19|20)\d{2}\b", " ", normalize_block_text(title_text))
    title_tokens = [tok for tok in title_text.split() if tok not in STOPWORDS]
    if not title_tokens:
        return keys

    if len(title_tokens) >= 2:
        keys.add(f"title_bigram:{title_tokens[0]}::{title_tokens[1]}")
        keys.add(f"title_bigram_tail:{title_tokens[-2]}::{title_tokens[-1]}")
    if len(title_tokens) >= 3:
        keys.add(f"title_prefix3:{'::'.join(title_tokens[:3])}")

    compact = "".join(title_tokens)
    if len(compact) >= 8:
        for idx in range(0, min(len(compact) - 3, 12), 2):
            gram = compact[idx:idx + 4]
            if len(gram) == 4:
                keys.add(f"title_c4:{gram}")

    return keys


def _person_block_keys(row: pd.Series, exclude_cols: set[str]) -> set[str]:
    keys: set[str] = set()
    year = _year_value(row, exclude_cols)
    for col in _person_columns(row, exclude_cols):
        ...
        if not chunks:
            chunks = [normalized]
        for chunk in chunks:
            tokens = [tok for tok in chunk.split() if len(tok) >= 2 and tok not in STOPWORDS]
            if not tokens:
                continue
            surname = tokens[0] if "," in raw else tokens[-1]
            if len(surname) >= 3:
                keys.add(f"person_surname:{surname}")
                if year:
                    keys.add(f"person_year:{surname}::{year}")
            if len(tokens) >= 2:
                pair = f"{tokens[0]}::{tokens[-1]}"
                keys.add(f"person_edge:{pair}")
    return keys


def get_entity_tokens(row: pd.Series, exclude_cols: set[str]) -> set[str]:
    """Collect base tokens plus robust blocking signatures for one entity row."""
    tokens: set[str] = set()
    for col, val in row.items():
        ...
        if isinstance(val, str) and val:
            tokens |= tokenize(val)
    tokens |= _title_block_keys(row, exclude_cols)
    tokens |= _person_block_keys(row, exclude_cols)
    return tokens
...
def build_inverted_index(
    ...
    *,
    ...
    progress = ProgressBar(progress_label, len(df), color="34", unit_label="entities") if progress_label else None
...
def index_to_blocks_df(
    ...
    *,
    ...
    rows = []
    progress = ProgressBar(progress_label, len(index), color="34", unit_label="blocks") if progress_label else None
    for idx, (token, entries) in enumerate(index.items(), start=1):
        for (eid, src) in entries:
            rows.append({"block_id": token, "entity_id": eid, "source": src})
        if progress:
            progress.update(idx)
    if progress:
        progress.close()
...
def run_token_blocking(
    ...
    index = build_inverted_index(
        ...
        entity_id_col,
        source_col,
        progress_label="  [BLOCK] Tokenisation + index",
    )
    print(f"  [BLOCK] Distinct tokens: {len(index)}")

    index = apply_block_size_limit(index, max_block_size)

    df_blocks = index_to_blocks_df(index, progress_label="  [BLOCK] Materialisation des blocks")
