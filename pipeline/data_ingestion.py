...
DBLP_SCHOLAR_RAW_DIR  = os.environ.get("DBLP_SCHOLAR_DIR",
    os.path.join(RAW_DIR, "DBLP-Scholar"))
...
def load_dblp_scholar(raw_dir: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Load the DBLP-Scholar bibliographic ER benchmark.

    Expected files:
      DBLP1.csv   — 2,616 DBLP records  (id, title, authors, venue, year)
      Scholar.csv — 64,263 Scholar records (id, title, authors, venue, year)
      DBLP-Scholar_perfectMapping.csv — ground truth (idDBLP, idScholar)

    This is a hard benchmark: large size asymmetry (2.6k vs 64k),
    noisy Scholar metadata, 5,347 true matches in 168M brute-force pairs.
    """
    dblp_path    = os.path.join(raw_dir, "DBLP1.csv")
    scholar_path = os.path.join(raw_dir, "Scholar.csv")
    truth_path   = os.path.join(raw_dir, "DBLP-Scholar_perfectMapping.csv")

    for path in [dblp_path, scholar_path]:
        if not os.path.exists(path):
            raise FileNotFoundError(f"[dblp_scholar] Cannot find: {path}")

    df_dblp = pd.read_csv(dblp_path, encoding="latin-1", dtype=str)

    # Scholar.csv has a UTF-8 BOM; utf-8-sig strips it so the 'id' column
    # name is parsed correctly (latin-1 decodes the BOM as 'ï»¿' garbage).
    try:
        df_scholar = pd.read_csv(scholar_path, encoding="utf-8-sig", dtype=str)
    except UnicodeDecodeError:
        df_scholar = pd.read_csv(scholar_path, encoding="latin-1", dtype=str)
        df_scholar.columns = [c.replace("ï»¿", "").strip('"') for c in df_scholar.columns]

    if os.path.exists(truth_path):
        df_truth = pd.read_csv(truth_path, encoding="utf-8", dtype=str)
        print(f"  [LOAD] Ground truth: {len(df_truth)} matches from perfectMapping")
    else:
        df_truth = pd.DataFrame(columns=["idDBLP", "idScholar"])
        print(f"  [WARN] No perfectMapping found in {raw_dir}")

    return df_dblp, df_scholar, df_truth
...
DATASET_REGISTRY: dict[str, dict] = {
    ...
    "dblp_scholar": {
        "loader":       load_dblp_scholar,
        "raw_dir":      DBLP_SCHOLAR_RAW_DIR,
        "source1":      "dblp",
        "source2":      "scholar",
        "data_type":    "Structured CSV",
        "description":  "Bibliographic ER benchmark — DBLP vs Google Scholar (hard: 2.6k×64k)",
        "truth_col_s1": "idDBLP",
        "truth_col_s2": "idScholar",
        "uri_cols_s1":  [],
        "uri_cols_s2":  [],
    },
    "rexa_dblp": {
...
# ── Extra dataset registry (auto-loaded from pipeline/extra_datasets.json) ───

_EXTRA_REGISTRY_PATH = os.path.join(os.path.dirname(__file__), "extra_datasets.json")


def _make_extra_loader(clean_dir: str):
    """Return a loader that reads pre-cleaned CSVs from *clean_dir*."""
    def _loader(_raw_dir: str):
        s1 = pd.read_csv(os.path.join(clean_dir, "cleaned_source1.csv"), dtype=str).fillna("")
        s2 = pd.read_csv(os.path.join(clean_dir, "cleaned_source2.csv"), dtype=str).fillna("")
        gt_path = os.path.join(clean_dir, "ground_truth.csv")
        gt = pd.read_csv(gt_path, dtype=str).fillna("") if os.path.exists(gt_path) else pd.DataFrame()
        return s1, s2, gt
    return _loader


def _merge_extra_registry():
    """Read pipeline/extra_datasets.json and merge entries into DATASET_REGISTRY."""
    if not os.path.exists(_EXTRA_REGISTRY_PATH):
        return
    try:
        with open(_EXTRA_REGISTRY_PATH) as f:
            extra = json.load(f)
    ...
    for name, cfg in extra.items():
        if name in DATASET_REGISTRY:
            continue
        clean_dir = os.path.join(CLEAN_DIR, name)
        DATASET_REGISTRY[name] = {
            "loader":       _make_extra_loader(clean_dir),
            "raw_dir":      clean_dir,          # checked for existence in run_ingestion
            "source1":      cfg.get("source1", "source1"),
            "source2":      cfg.get("source2", "source2"),
            "data_type":    cfg.get("data_type", "Auto-detected"),
            "description":  cfg.get("description", f"Auto-integrated dataset: {name}"),
            "truth_col_s1": cfg.get("truth_col_s1", "idS1"),
            "truth_col_s2": cfg.get("truth_col_s2", "idS2"),
            "uri_cols_s1":  [],
            "uri_cols_s2":  [],
            "pre_cleaned":  True,
        }
...
# Merge auto-integrated datasets from pipeline/extra_datasets.json
_merge_extra_registry()
...
def run_ingestion(
    ...
    progress = ProgressBar(f"[INGEST:{dataset}]", 4, color="33", unit_label="phases")
    ...
    # Load (and optionally clean) DataFrames
    progress.update(0, detail="chargement brut")
    df1_raw, df2_raw, df_truth = cfg["loader"](raw_dir)
    progress.update(1, detail="chargement brut terminé")

    if cfg.get("pre_cleaned"):
        # Data was already cleaned by integrate_dataset.py — use as-is
        df1, df2 = df1_raw, df2_raw
        print(f"  [INGEST] Pre-cleaned dataset — skipping normalisation")
        progress.update(2, detail="normalisation ignorée")
    else:
        progress.update(1, detail=f"nettoyage {cfg['source1']}")
        df1 = clean_dataframe(df1_raw, source_name=cfg["source1"],
                              uri_columns=cfg["uri_cols_s1"])
        progress.update(2, detail=f"nettoyage {cfg['source2']}")
        df2 = clean_dataframe(df2_raw, source_name=cfg["source2"],
                              uri_columns=cfg["uri_cols_s2"])
        progress.update(3, detail="nettoyage terminé")
    ...
    print(f"  [SAVE] {s1_out}")
    print(f"  [SAVE] {s2_out}")
    progress.update(4, detail="fichiers nettoyés sauvegardés")
    progress.close()
