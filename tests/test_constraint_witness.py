...
def test_semantic_rescue_uses_full_row_text_when_title_is_missing():
    source1 = pd.DataFrame(
        [
            {
                "id": "a1",
                "title": "Love's Enduring Promise",
                "actor_name": "January Jones",
                "director_name": "Michael Landon Jr",
            }
        ]
    )
    source2 = pd.DataFrame(
    ...
    produced = set(zip(rescue["id_A"], rescue["id_B"]))
    assert ("a1", "b1") in produced
    assert stats["semantic_rescue_pairs"] >= 1
